...
def home(request):
    ...
    context['groups'] = models.Groups.objects.filter(delete_flag = 0).all().count()
...
def per_group(request):
    ...
    if 'group' in request.GET and 'status' in request.GET:
        try:
            filters = {
                'group__id': request.GET['group'],
                'status': request.GET['status']
            }

            # Optional marital_status filter
            if 'marital_status' in request.GET and request.GET['marital_status']:
                filters['marital_status'] = request.GET['marital_status']
                context['marital_status'] = request.GET['marital_status']

            context['members'] = models.Members.objects.filter(**filters).all()
            context['selected_group'] = models.Groups.objects.get(pk=request.GET['group'])
            context['status'] = request.GET['status']

        except Exception as err:
            print("Error filtering members:", err)
